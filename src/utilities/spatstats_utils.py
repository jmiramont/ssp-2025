...
def get_white_noise_zeros(stft_params, complex_noise=False):
    ...
    _, pos,_ = one_sided_spectrogram_and_zeros(wnoise)
...
def compute_monte_carlo_sims(signal,
    ...
    # Computes the spectrogram and its zeros.
    _, pos_exp,_ = one_sided_spectrogram_and_zeros(signal)
    T = np.sqrt(Nfft)
...
def generate_white_noise_zeros_pp(N, nsim, Nfft=None,complex_noise=False, parallel=False):
    # STFT parameters.
    if Nfft is None:
        Nfft = 2*N
...
def compute_rank_envelope_test(signal,
    ...
    if N>4095:
        pos = compute_spectrogram_and_zeros(signal)
    else:
        _, pos, _ = one_sided_spectrogram_and_zeros(signal)
